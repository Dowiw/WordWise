...
void* saveWordThreadFunc(void* arg) {
	...
	std::cout << "[Thread] Saving stats for word: '" << args->germanWord << "'...\n";
	pthread_mutex_unlock(&cout_mutex);
	wordDB.updateWordStats(args->germanWord, args->wasCorrect);
	usleep(300000); // 0.3s for demo
	auto end = std::chrono::high_resolution_clock::now();
	double elapsed_ms = std::chrono::duration<double, std::milli>(end - start).count();
	pthread_mutex_lock(&cout_mutex);
	std::cout << "[Thread] Done saving: '" << args->germanWord << "' (" << elapsed_ms << " ms)\n";
	pthread_mutex_unlock(&cout_mutex);
	delete args;
	return nullptr;
}
...
void saveWordsInParallel(const std::vector<std::pair<std::string, bool>>& wordResults) {
	std::vector<pthread_t> threads(wordResults.size());
	...
	std::cout << "All word stats saved in parallel!\n";
}
